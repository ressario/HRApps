...
function initialSetting() {
    ...
    if (!samplingNumberModel) {
        ...
        document.getElementById('txtSamplingReqNumber').value = 'SOD/NO-' + generateAutoNumber($('#txtRequestor').val());
        ...
        if (approvalStatusModel !== "0") {
            ...
            $('#divbtnApprove').hide();
            $('#divbtnReject').hide();
            $('#txtSamplingReqNumber').val('SOD/NO-' + generateAutoNumber($('#txtRequestor').val()));
        }
    }
...
function bindGridSampling(response) {
    _gridSamplingDirect = $("#gridSamplingDirect").kendoGrid({
        dataSource: {
            ...
            pageSize: 10,
            aggregate: [
                //{ field: "SamplingPrice", aggregate: "sum" },
        ...
        columns: [
            ...
            //},
            { field: "SamplingPrice", title: "Sampling Price / Case (Rp)", format: "{0:#,0.00}", width: "70px", attributes: { class: "text-right" } },
            {
...
function saveAttachment() {
    $.ajax({
        type: "post",
        url: MAA_API_Server + 'SamplingDirect/UploadFiles',
        contentType: false,
        processData: false,
        data: data,
...
function SaveSampling() {
    ...
    $.ajax({
        type: "POST",
        url: url_Web + 'SamplingDirect/SaveSamplingRequest',
        data: save_Sampling,
        beforeSend: function () {
            //loading icon...
        },
...
function bindDMENumber() {
    ...
    $.ajax({
        type: "GET",
        url: MAA_API_Server + 'SamplingDirect/GetActiveDMERequest',
        success: function (msg) {
            
            for (var i = 0; i < msg.length; i++) {
                dmeNumber.push(msg[i].dmeRequestNumber);
            }
            console.log('bind: ' + dmeNumber);

            autocomplete(document.getElementById("txtDMENumber"), dmeNumber);
        },
        error: function (data) {
            alert('Something Went Wrong');
        }
...
function initFileUpload() {
    ...
    $("#btnExcelUpload").on('click', function (e) {
        ...
        $.ajax({
            type: "POST",
            url: url_Web + "SamplingDirect/UploadExcel",
            data: model,
            processData: false,
            contentType: false,
            beforeSend: function () {
...
function getDetailSamplingList(samplingnum) {
    ...
    $.ajax({
        type: "GET",
        url: MAA_API_Server + 'SamplingDirect/GetDetailSamplingRequestDataByRequestNumber',
